...
const Avalanche = async (
  ...
  const address: string = getAddress(Networks[2].name)
    ? getAddress(Networks[2].name)
    : log.error(Networks[2].name + ": No suitable account to use") &&
      process.exit(1);
  const provider = getProvider(2);
  const signer = getSigner(Networks[2].name, provider);
  const systemGas = {
    gasPrice:
  ...
  if (networkSettings.showBlockNumber) {
    setInterval(async () => {
      log.info(Networks[2].name + ": Block: " + (await getBlock(provider)));
